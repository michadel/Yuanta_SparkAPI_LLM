# 元大證券 SparkAPI (原 OneAPI) IO 規格文件集
...
- `GetStockInformation_IO_Spec.md`: 股票基本資料查詢。
- `SendStockOrder_IO_Spec.md`: 下單功能。
- `GetStkTickDetail_IO_Spec.md`: 即時逐筆成交明細。
- ...以及其他相關的股票歷史與交易紀錄規格。

### 2. 期貨與選擇權 (Futures & Options)
- `SendFutureOrder_IO_Spec.md`: 期貨下單。
- `GetFutStoreSummary_IO_Spec.md`: 期貨部位總結。
- ...以及相關的保證金與利息規格。

### 3. 市場行情與訂閱 (Market Data & Subscription)
- `SubscribeStockInformation_IO_Spec.md`: 即時股票資訊訂閱。
- `GetKLine_IO_Spec.md`: K 線圖資料查詢。
- `GetQuoteList_IO_Spec.md`: 行情列表查詢。

### 4. 帳戶與資產管理 (Account & Asset Management)
- `GetBankBalance_IO_Spec.md`: 銀行帳戶餘額查詢。
- `GetUnrealizedGainLossDetail_IO_Spec.md`: 未實現損益明細。

### 5. 自動交易策略與演算法 (Algo & Strategy)
- `SendAlgoCOOdrStrategy.md`: 發送演算法策略指令。
- `DeleteAlgoCOOdrStrategy.md`: 刪除演算法策略。
...
**範例指令邏輯：**
1. **識別意圖**：判斷使用者想要執行的操作（例如：「查詢我的股票部位」）。
2. **匹配 API**：從文件集中找到對應的 API 規格（例如：`GetStoreSummary_IO_Spec.md`）。
3. **構建請求**：根據該文件的 `Input` 規範，提取必要的參數並組成 JSON 物件。
4. **解析回應**：當收到 API 回傳結果時，參考該文件的 `Output` 規格來解釋數據內容。
